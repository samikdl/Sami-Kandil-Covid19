...
export default function KPICard({
  ...
  const trendColor = typeof trend === 'number' && trend > 0 ? 'text-rose-400' : 'text-emerald-400';
  const trendRotate = typeof trend === 'number' && trend < 0 ? 'rotate-180' : '';

  return (
    <div className={`group relative overflow-hidden rounded-2xl border border-white/10 bg-gradient-to-br from-white/[0.07] to-white/[0.02] p-6 shadow-2xl backdrop-blur-sm transition-all hover:border-white/20 hover:shadow-2xl ${accentRing}`}>
        ...
        <div className="flex items-start justify-between">
          ...
          {typeof trend === 'number' && (
            <div className={`flex items-center gap-1 text-xs ${trendColor}`}>
              <TrendingUp className={`h-3 w-3 ${trendRotate}`} />
